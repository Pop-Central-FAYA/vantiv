...
class BroadcasterUserReportsController extends Controller
{
    ...
    public function HVCdata(Datatables $datatables, Request $request)
    {
        ...
        if($request->has('start_date') && $request->has('stop_date'))
        {
            ...
            $campaign = Utilities::switch_db('api')->select("SELECT COUNT(id) as total_campaign, agency_broadcaster, agency, time_created as `time`, id, walkins_id, SUM(adslots) as total_adslot from campaignDetails WHERE broadcaster = '$broadcaster' AND agency = '$broadcaster_user' walkins_id != '' AND time_created BETWEEN '$start' AND '$end' GROUP BY walkins_id ");
            ...
            $campaign = Utilities::switch_db('api')->select("SELECT COUNT(id) as total_campaign, agency_broadcaster, agency, time_created as `time`, id, walkins_id, SUM(adslots) as total_adslot from campaignDetails WHERE broadcaster = '$broadcaster' AND agency = '$broadcaster_user' AND walkins_id != '' GROUP BY walkins_id ");
            foreach ($campaign as $c) {
                $user_broad = Utilities::switch_db('api')->select("SELECT * from users where id = (SELECT user_id from walkIns where id = '$c->walkins_id')");
                if($user_broad){
                    $user_details = $user_broad[0]->firstname . ' ' . $user_broad[0]->lastname;
    ...
    public function PIdata(Datatables $datatables, Request $request)
    {
        ...
        if($request->has('start_date') && $request->has('stop_date'))
        {
            $inv = Utilities::switch_db('api')->select("SELECT * from invoiceDetails WHERE broadcaster_id = '$broadcaster' and agency_id = '$broadcaster_user' AND time_created BETWEEN '$start' AND '$end'");
            foreach ($inv as $i)
            {
                $invoice_det = Utilities::switch_db('api')->select("SELECT * from invoices where id = '$i->invoice_id'");
                $campaign_id = $invoice_det[0]->campaign_id;
            ...
            $inv = Utilities::switch_db('api')->select("SELECT * from invoiceDetails WHERE broadcaster_id = '$broadcaster'AND agency_id = '$broadcaster_user' ");
            foreach ($inv as $i)
            {
                $invoice_det = Utilities::switch_db('api')->select("SELECT * from invoices where id = '$i->invoice_id'");
                $campaign_id = $invoice_det[0]->campaign_id;
    ...
    public function psData(Datatables $datatables, Request $request)
    {
        ...
        if($request->has('start_date') && $request->has('stop_date'))
        {
            ...
            $periodic = Utilities::switch_db('api')->select("SELECT id, campaign_id, agency, adslots, `name`, brand, walkins_id, time_created as days from campaignDetails WHERE broadcaster = '$broadcaster' AND agency = '$broadcaster_user' AND walkins_id != '' AND adslots != 0 AND ( time_created BETWEEN '$start' AND '$end') ORDER BY time_created desc");
            foreach ($periodic as $pe)
            {
                $payment = Utilities::switch_db('api')->select("SELECT * from payments where campaign_id = '$pe->campaign_id'");
                $payment_id = $payment[0]->id;
                $price = Utilities::switch_db('api')->select("SELECT amount, time_created as days from paymentDetails WHERE payment_id = '$payment_id'");
            ...
            $ads = [];
            $j = 1;
            $periodic = Utilities::switch_db('api')->select("SELECT id,campaign_id,adslots,agency, `name`, brand, walkins_id, time_created as days from campaignDetails WHERE broadcaster = '$broadcaster' AND agency = '$broadcaster_user' AND walkins_id != '' AND adslots != 0 ORDER BY time_created desc");
            foreach ($periodic as $pe)
            {
                $payment = Utilities::switch_db('api')->select("SELECT * from payments where campaign_id = '$pe->campaign_id'");
                $payment_id = $payment[0]->id;
    ...
    public function tvcData(Datatables $datatables, Request $request)
    {
        ...
        if($request->has('start_date') && $request->has('stop_date'))
        {
            ...
            $camp_vol = Utilities::switch_db('api')->select("SELECT COUNT(id) as volume, time_created as month from campaignDetails where broadcaster = '$broadcaster' AND agency = '$broadcaster_user' AND time_created BETWEEN '$start' AND '$end' GROUP BY DATE_FORMAT(time_created, '%Y-%m') ORDER BY time_created desc");
            $c_vol = [];
            $j = 1;
            ...
            $camp_vol = Utilities::switch_db('api')->select("SELECT COUNT(id) as volume, DATE_FORMAT(time_created, '%M, %Y') as `month` from campaignDetails where broadcaster = '$broadcaster' AND agency = '$broadcaster_user' GROUP BY DATE_FORMAT(time_created, '%Y-%m') ORDER BY time_created desc");
            $c_vol = [];
            $j = 1;
            foreach ($camp_vol as $ca)
            {
    ...
    public function hpdData(Datatables $datatables, Request $request)
    {
        ...
        if($request->has('start_date') && $request->has('stop_date'))
        {
            ...
            $all_dayp = [];
            $dayp_namesss = [];
            $slots = Utilities::switch_db('api')->select("SELECT adslots_id from campaignDetails where broadcaster = '$broadcaster' AND agency = '$broadcaster_user' and time_created BETWEEN '$start' AND '$end'");
    ...
    public function hpdaysData(DataTables $datatables, Request $request)
    {
        ...
        if($request->has('start_date') && $request->has('stop_date'))
        {
            ...
            $days = Utilities::switch_db('api')->select("SELECT COUNT(id) as tot_camp, time_created as days from campaignDetails where broadcaster = '$broadcaster' AND agency = '$broadcaster_user' AND day_parts != '' AND time_created BETWEEN '$start' AND '$end' GROUP BY DATE_FORMAT(time_created, '%a'), broadcaster ORDER BY WEEK(time_created) desc LIMIT 1");
            $day_name = [];
            $b = [];
            ...
            $days = Utilities::switch_db('api')->select("SELECT COUNT(id) as tot_camp, time_created as days from campaignDetails where broadcaster = '$broadcaster' AND agency = '$broadcaster_user' AND day_parts != '' GROUP BY DATE_FORMAT(time_created, '%a'), broadcaster ORDER BY WEEK(time_created) desc ");
            $day_name = [];
            $b = [];
            for($j = 0; $j < count($days); $j++)
            {
