...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        //paid invoices
        $invoice_array = [];
        $inv = Utilities::switch_db('reports')->select("SELECT * from invoices WHERE broadcaster_id = '$broadcaster' LIMIT 10");
        foreach ($inv as $i)
        {
        ...
        $camp_vol = Utilities::switch_db('reports')->select("SELECT COUNT(id) as volume, DATE_FORMAT(time_created, '%M, %Y') as `month` from campaigns where broadcaster = '$broadcaster' GROUP BY DATE_FORMAT(time_created, '%Y-%m') ORDER BY time_created desc");
        $c_vol = [];
        $c_month = [];
        ...
        $dayp = Utilities::switch_db('reports')->select("SELECT COUNT(id) as campaigns, DATE_FORMAT(time_created, '%a') as time_created, day_parts 
        ...
        {
            $day_p = $day->day_parts;
            $query = Utilities::switch_db('reports')->select("SELECT day_parts from dayParts WHERE id IN ('$day_p')");
        ...
        $days = Utilities::switch_db('reports')->select("SELECT COUNT(id) as tot_camp, DATE_FORMAT(time_created, '%a') as days from campaigns where broadcaster = '$broadcaster' AND day_parts != '' GROUP BY DATE_FORMAT(time_created, '%a'), broadcaster ORDER BY WEEK(time_created) desc LIMIT 1");
        $day_name = [];
        $b = [];
        ...
        $months = [];
        $slot = [];
        $periodic = Utilities::switch_db('reports')->select("SELECT count(id) as tot_camp, SUM(adslots) as adslot, time_created as days from campaigns WHERE broadcaster = '$broadcaster' GROUP BY DATE_FORMAT(time_created, '%Y-%m') ORDER BY time_created desc");
        foreach ($periodic as $pe)
        {
            $price = Utilities::switch_db('reports')->select("SELECT SUM(amount) as total_price, time_created as days from payments WHERE broadcaster = '$broadcaster' AND time_created = '$pe->days' GROUP BY DATE_FORMAT(time_created, '%Y-%m') ORDER BY time_created desc");
            $ads[] = [
...
//        Inventory fill rate
        ...
        $rate_c = [];
        $get_day_id = Utilities::switch_db('reports')->select("SELECT id as day_id from days WHERE day = '$today_day'");
