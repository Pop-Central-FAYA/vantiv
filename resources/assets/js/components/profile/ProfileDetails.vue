             ...
             <v-layout  style="height: 80px; margin-Top: 10px; margin-right: -70px;"> 
                <v-flex md11 my-1>
                    ...
                    <p class="weight_medium small_faint">
                     <span v-for="(list, index) in user.role_name" v-bind:key="index"> <span>{{list.label}}</span> <span v-if="index+1 < user.role_name.length">, </span>  </span>    
                    </p>
                </v-flex>
                 <v-flex md2 my-1>
                    <b-img thumbnail fluid :src="user.avatar"  style="width: 70px; height: 70px" alt="Image 1"></b-img>       
                </v-flex>
            </v-layout>
        </v-flex>
    </v-layout>
