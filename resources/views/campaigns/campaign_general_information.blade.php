...
@section('title')
    <title>Torch | Campaign General Information</title>
@stop
    ...
    <!-- subheader -->
        <div class="sub_header clearfix mb pt">
            <div class="column col_6">
                <h2 class="sub_header">Campaign General Information</h2>
            </div>
        </div>
        ...
        <div class="the_frame clearfix mb border_top_color load_stuff">
            ...
            <div class="margin_center col_7 clearfix pt4 create_fields">
                ...
                <form method="POST" action="{{ route('campaign.store_campaign_general_information') }}">
                    {{ csrf_field() }}
                                    ...
                                    <select name="brand" id="brand">
                                        @foreach($brands as $brand)
                                            <option value="{{ $brand->id }}"
                                                    @if($campaign_general_information->brand === $brand->id)
                                                    selected
                                                @endif
                                            >{{ $brand->name }}</option>
                                        @endforeach
                            ...
                            @foreach($regions as $region)
                                <li class="col_4 column m-b">
                                    <input name="region[]" value="{{ $region->id }}" data-region="{{ $region->id }}"
                                           ...
                                           @endforeach
                                           @endif
                                           type="checkbox" class="checkbox_region checked{{ $region->id }}" id='{{ $region->id }}'>
